    ...
    public class Provider : IProvider
    {
        ...
        public async Task Register(string savedState, Action<string, ProviderState> onResult)
        {
            try
            {
                ...
                var oldToken = await store.GetAsync<Google.Apis.Auth.OAuth2.Responses.TokenResponse>("token");
                var secret = new ClientSecrets() { ClientId = Obscure.Decode(Constants.AppKey), ClientSecret = Obscure.Decode(Constants.AppSecret) };
                var credential = await GoogleWebAuthorizationBroker.AuthorizeAsync(secret, Constants.Scopes, "token", CancellationToken.None, store);
